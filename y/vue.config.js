  ...
  if (isProd()) {
    plugins.push(
      ...
      new webpack.BannerPlugin({
        banner: `@author: Whale FE${N}@version: ${pkg.version}${N}@description: Build time ${formatDate(
          new Date(),
          'yyyy-MM-dd HH:mm:ss'
        )}          `
      }),
      new WebpackManifestPlugin({
        fileName: path.resolve(__dirname, 'dist', `manifest.${Date.now()}.json`),
  ...
  configureWebpack: () => ({
    ...
    resolve: {
      alias: {
        ...
        'native-bridge-methods': resolve(
          'node_modules/@winner-fed/native-bridge-methods/dist/native-bridge-methods.esm'
        )
      }
    },
    plugins: genPlugins(),
